...
(
    ...
    [Picture] VARBINARY(MAX), -- FILESTREAM for large images
    [Private] BIT,            --hides it from everyone else's discover screen -- used with HouseholdId to keep a recipie to your household only
    ComplexityId TINYINT,     -- fk to  RecipiesComplexity
    [CreatedDate] DATETIME,   -- default db level to today GETUTCDATE()
    [CreatedBy] NVARCHAR(36), -- FK to Houses.Occupants.OccupantId
    [ModifiedDate] DATETIME,  -- default db level to today GETUTCDATE()
    [ModifiedBy] NVARCHAR(36), -- FK to Houses.Occupants.OccupantId
);

CREATE TABLE Food.RecipiesComplexityLookUp
(
    ComplexityId TINYINT,
    [Name] NVARCHAR(50) -- Easy, Meduim, Hard
);

CREATE TABLE Food.RecipieSteps
(
    RecipieStepId INT IDENTITY(1, 1),
    RecipieId INT,            -- FK to Recipies + index
    Step NVARCHAR(4000),
    [CreatedDate] DATETIME,   -- default db level to today GETUTCDATE()
    [CreatedBy] NVARCHAR(36), -- FK to Houses.Occupants.OccupantId
    [ModifiedDate] DATETIME,  -- default db level to today GETUTCDATE()
...
(
    ...
    RecipieId INT,            --FK to Recipies + index
    [Name] NVARCHAR(200),     -- not FK as user sub
    Quantity DECIMAL(10, 2),
    UnitsID INT,              -- FK to UnitsLookup, optional field
    [CreatedDate] DATETIME,   -- default db level to today GETUTCDATE()
    [CreatedBy] NVARCHAR(36), -- FK to Houses.Occupants.OccupantId
    [ModifiedDate] DATETIME,  -- default db level to today GETUTCDATE()
    [ModifiedBy] NVARCHAR(36), -- FK to Houses.Occupants.OccupantId
...
(
    ...
    HouseholdId INT, -- fk to households -- index
    RecipieId INT    -- fk to recipies  -- index
);
...
(
    ...
    CategoryId INT, -- FK to categoriesLookUp 
    RecipieId INT   -- FK to recipies
);
...
(
    ...
    [Name] NVARCHAR(50),      -- front end default to date to id list easily
...
CREATE TABLE Food.MealPlan (
	...
	HouseholdId INT, -- Link to households
	RecipieId INT, -- Link to receipies
	DateId INT
);
...
CREATE TABLE Food.DateLookup (
	...
	[Date] NVARCHAR(10) -- format dd/MM/YYYY
)
